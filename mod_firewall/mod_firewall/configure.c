...
//			printf(" first in getpara: %s\n",argv[optind]);
        ...
        case 'b':
                ...
                controlled_time_begin=time_info_begin.tm_hour*60+time_info_begin.tm_min;
                break;
        case 'e':
                ...
                controlled_time_flag=1;
                controlled_time_end=time_info_end.tm_hour*60+time_info_end.tm_min;
                break;
		case 't':
        ...
        case 'i':
                ...
                controlled_interface=tmp_interface;
                break;
...
//	printf("input info: p = %d, x = %d y = %d m = %d n = %d \n", controlled_protocol,controlled_saddr,controlled_daddr,controlled_srcport,controlled_dstport);
	...
	if (stat("/dev/controlinfo",&buf) != 0){	//这段代码是在检查文件/dev/controlinfo的状态。stat 函数用于获取文件的状态信息，并将结果存储在提供的结构体变量 buf 中。
