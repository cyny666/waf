    ...
    def add_get():
        ...
        if check == 1:
            ...
            if flag == 0:
                ...
                rule = [number, SADDR_text, SPORT_text, DADDR_text, DPORT_text, interface_text, time_begin_text,
                        time_end_text, protocol_text, icmp_subtype_text]
                number += 1
            ...
            if check == 1:
                ...
                if flag == 0:
                    ...
                    if not re.search(r"\d", interface_text):
                        ...
                        interface_text = "  " + interface_value
                    rule = [number, SmodifyR_text, SPORT_text, DmodifyR_text, DPORT_text, interface_text, time_begin_text,
                            time_end_text, protocol_text, icmp_subtype_text]
                    number += 1
                    table.insert('', END, values=rule)
                else:
...
modify_rules = Button(root, text="修改规则", font=module)
...
# 插入规则表格
columns = ['Number', 'SADDR', 'SPORT', 'DADDR', 'DPORT', 'INTERFACE', 'TIME_BEGIN', 'TIME_END', 'PROTOCOL',
           'icmp subtype']
table = ttk.Treeview(
